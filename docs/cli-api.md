...
`zeroday report` (`zeroday.report/v1`) turns existing prove-doors JSON and/or SARIF into a short CISO summary (optional historical gpu-evidence footnote; optional `--top N` truncates ranked findings); localization ≠ exploitability; `runpod: false`. Desk: `POST /api/report` → download `report.json` + `report.md`.
