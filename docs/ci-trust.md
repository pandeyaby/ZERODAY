...
| Stranger prove-doors job | `stranger-verify` — runs `npm run --silent stranger:verify -- --json` (Door A keyless + Door B citation-only; no GPU) **and** `npm run --silent prove-doors -- --json` (Door A + cassette; Door B skipped); uploads artifacts `stranger-verify-json` + `prove-doors-json` (**not** vuln / AUROC proof) |
...
| **`prove-doors`** (`npm run prove-doors -- --json`) | In-process Door A + cassette:replay; Door B skipped without `--live-url`; CI uploads `prove-doors-json` (not vuln / AUROC proof) |
