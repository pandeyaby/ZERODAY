...
| **`evidence-pack`** (`npm run evidence-pack -- --json --out out/evidence`) | CI builds design-partner folder (`prove-doors.json` + historical `gpu-evidence.json` + `manifest.json`); does not start RunPod; uploads `evidence-pack` (not live GPU / AUROC proof) |
