...
|------|------|----------------------|
| `report.sarif` | GitHub Code Scanning | Upload with `github/codeql-action/upload-sarif` (our Action already does this) or your own workflow. Severity is **note**. `partialFingerprints` for dedupe. Omit invented line regions. |
