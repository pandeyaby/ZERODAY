...
|------|------|----------------------|
| `report.sarif` | GitHub Code Scanning | Upload with `github/codeql-action/upload-sarif` (our Action already does this), `npm run upload-sarif -- --sarif report.sarif` (`--dry-run` for CI; live needs `security_events: write`), or your own workflow. **Desk** `POST /api/upload-sarif` is dry-run only (never calls GitHub); live upload stays CLI with `security_events: write`. Localization SARIF only — not exploit proof. Severity is **note**. `partialFingerprints` for dedupe. Omit invented line regions. |
