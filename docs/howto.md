...
cassette:replay + Door D historical A40 evidence + Door E upload-sarif dry-run + optional Door B; B `skipped` without liveUrl). Download **prove-doors.json** from the last Run-all response (same as CLI `--out`). Door E = dry-run Code Scanning check, not live upload. CLI:
