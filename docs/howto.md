...
[Open in app](http://localhost:3333/play) · War Room tab **How to use** (shortcut `h`)
...
Buttons run existing fixture paths: locate CWE-89, classify each scenario, mixed `zeroday demo`. The UI shows a SARIF summary, Splunk-shaped JSON snippet, and CISO markdown. No live telemetry, no simulated attacks, no exploits, no gated weights.
