...
import {
  ...
  type EvidencePackDownloadPayload,
} from "@/desk/evidence-pack-download";
import {
...
type EvidencePackJson = EvidencePackDownloadPayload & {
  code?: string;
};
...
export function ProveDoorsPanel() {
  ...
  const runEvidencePackDownload = useCallback(async () => {
    setPackBusy(true);
    setPackError(null);
    setPackResult(null);
    try {
      ...
      if (!res.ok || json.ok === false) {
        setPackError(json.error || `HTTP ${res.status}`);
        setPackResult(null);
        return;
      }
      setPackResult(json);
      downloadEvidencePackFiles(json);
    } catch (e) {
  ...
  return (
    <div className="space-y-4 animate-fade-up" data-testid="prove-doors-panel">
      ...
      <div
        className="panel rounded-lg p-4"
        data-testid="prove-doors-evidence-pack-card"
      >
        <div className="panel-header !px-0 !pt-0 !border-0">
          <span className="text-sm font-display tracking-wide flex items-center gap-2">
            <FileJson size={14} /> Download evidence-pack
          </span>
          <Badge tone="ok">POST {EVIDENCE_PACK_API_PATH}</Badge>
        </div>
        <p className="text-xs text-[var(--muted)] mt-2 mb-3">
        ...
        <div className="flex flex-wrap items-center gap-2 mt-3">
          <Button
            ...
            size="md"
            disabled={anyBusy}
            onClick={() => void runEvidencePackDownload()}
            data-testid="prove-doors-evidence-pack-run"
            aria-label="Build and download evidence-pack via Desk API"
          >
            {packBusy ? (
              <Loader2 size={14} className="animate-spin" />
            ) : (
        ...
        {packError ? (
          <p
            className="mt-3 text-sm text-[var(--danger)]"
            data-testid="prove-doors-evidence-pack-error"
            role="alert"
          >
            {packError}
          </p>
        ...
        {packResult ? (
          <div className="mt-3" data-testid="prove-doors-evidence-pack-result">
            <div className="flex flex-wrap items-center gap-2 mb-2">
              <Badge tone="ok">
                {String(packResult.schemaVersion ?? "evidence-pack")}
              </Badge>
              <Badge tone={packResult.ok ? "ok" : "warn"}>
                ok: {String(packResult.ok)}
              </Badge>
              <Badge tone="muted">
                startsRunPod: {String(packResult.startsRunPod ?? false)}
              </Badge>
              <Badge tone="warn">historical gpu-evidence</Badge>
            </div>
            <div className="flex flex-wrap items-center gap-2 mb-2">
              <Button
                type="button"
                ...
                variant="outline"
                onClick={() => {
                  downloadEvidencePackFiles(packResult);
                }}
                data-testid="prove-doors-evidence-pack-download"
                aria-label={`Re-download evidence-pack (${EVIDENCE_PACK_MANIFEST_DOWNLOAD_FILENAME} + doors)`}
              >
                <Download size={14} />
                Re-download pack files
              </Button>
              <CopyJsonButton
                value={serializeEvidencePackJson(
                  packResult.files?.[EVIDENCE_PACK_MANIFEST_DOWNLOAD_FILENAME] ??
                    packResult.manifest ??
                    packResult,
                )}
                ariaLabel={`Copy ${EVIDENCE_PACK_MANIFEST_DOWNLOAD_FILENAME} JSON`}
                testId="prove-doors-evidence-pack-copy"
              />
              <span className="text-[11px] text-[var(--muted)]">
                Same shape as CLI{" "}
                <code className="text-[var(--accent)]">{EVIDENCE_PACK_CMD}</code>
                {" · "}
            ...
            <pre
              ...
              data-testid="prove-doors-evidence-pack-json"
            >
              {JSON.stringify(
                packResult.files?.[EVIDENCE_PACK_MANIFEST_DOWNLOAD_FILENAME] ??
                  packResult.manifest ??
